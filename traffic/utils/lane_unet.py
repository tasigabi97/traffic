...
class Unet(Singleton):
    ...
    def batch_generator(self,database:LaneDB):
        grayscale_array_container, one_hot_container=zeros((self.batch_size,CAMERA_ROWS, CAMERA_COLS, 1),dtype=float64),zeros((self.batch_size,CAMERA_ROWS, CAMERA_COLS, self.category_num),dtype=float64)
        while True:
    ...
    def learning_rate_reducer(self):
        return ReduceLROnPlateau_ke(monitor=self.metrics[0],
